...
<a id="index-prompts"></a>
### P1–P12: Промпты

| ID | Название | Вход | Выход | Секция |
|----|----------|------|-------|--------|
| [P1](#p1) | Connection Note Generator | target_name, role, company, shared_context, recent_post, angle, my_role, my_company | 3 variants (A/B/C) ≤200 chars each | 4. Prompt Library |
| [P2](#p2) | Follow-Up DM Generator | name, context, days_since, recent_activity, value_offer, sequence_step | 2 variants (A/B) per step | 4. Prompt Library |
| [P3](#p3) | Recruiter Message Generator | recruiter_name, company, job_description, highlight_reel, target_role | Message + subject line variants | 4. Prompt Library |
| [P4](#p4) | Comment Generator | post_text, author, expertise_tags, tone, comment_goal | 3 variants (A/B/C) ≥40 words each | 4. Prompt Library |
| [P5](#p5) | Post Hook Generator | topic, target_audience, angle, format_preference, current_trend | 5 hooks + format + CTA | 5. Content Engine |
| [P6](#p6) | DM to Hiring Manager | hm_name, company, department, value_prop, referral_context | 2 cold + 1 warm variant | 4. Prompt Library |
| [P7](#p7) | Format Decision Engine | topic, goal, data_complexity, time_available, design_skill | Recommended format + rationale | 5. Content Engine |
| [P8](#p8) | Tone & Angle Selector | topic, experience_level, audience_size, controversy_tolerance, brand_goal | 3 tone/angle combos + virality estimate | 5. Content Engine |
| [P9](#p9) | Content Mix Rotator | last_5_posts, current_week_goal, upcoming_events, yesterday_top_comment, current_connections | Next post recommendation (tone/format/topic) | 5. Content Engine |
| [P10](#p10) | Full Post Generator | topic, format, tone, persona, target_audience, engagement_goal, word_count, hook_type | Full post structure (hook/body/CTA) + metadata | 5. Content Engine |
| [P11](#p11) | Performance Analyzer | last_post_metrics, engagement_rate, follower_growth, comment_sentiment, benchmark_data | Performance diagnosis + next actions | 5. Content Engine |
| [P12](#p12) | SSI Analyzer | ssi_score, component_scores, weekly_connections, acceptance_rate, posts_last_30_days, comments_last_30_days, profile_completeness | Component audit + 30-day forecast + triggers | 6. Metrics |

*Примечание: Профильные промпты (Headline Generator, About Generator) не входят в нумерацию P1–[P12](#p12), так как являются одноразовыми инструментами настройки профиля (Секция 1).*|

<a id="index-algorithms"></a>
### S1–S3: Алгоритмы Discovery

| ID | Название | Вход | Выход | Секция |
|----|----------|------|-------|--------|
| [S1](#s1) | Boolean Search Generator (Posts) | persona, skills, target_audience, post_type, exclude_terms | LinkedIn Native + Google X-Ray strings | 3. Discovery |
| [S2](#s2) | Engagement Scoring / Lead Scoring | networking_goal, target_role, geography, seniority, target_industry, target_companies, exclude_roles | Scored lead list + decision rules | 3. Discovery |
| [S3](#s3) | Content Calendar / Topic Discovery | author_expertise, author_geography, target_audience, competitor_posts, industry_trends | 3–5 topics for 2 weeks + gap analysis | 3. Discovery |

---

*Примечание: Нумерация P1–[P12](#p12) единая для всего документа. Профильные промпты (Headline Generator, About Generator) не входят в эту нумерацию.*
...
| Выступление / вебинар / подкаст → | Ссылка на запись или слайды | [P1](#p1) |
| LinkedIn-пост с высокой engagement (>100 reactions) → | Repost в Featured | [P1](#p1) |
| Сертификат (AWS, GCP, Databricks) → | Badge + credential ID | [P2](#p2) |
...
| 5+ элементов → | Удалить все [P2](#p2), оставить топ-3 по score |
...
> **Назначение:** Не расписание контента, а расписание запуска инструментов/промптов. Каждое действие символически ссылается на промпт (P1–[P12](#p12)) или системный промпт (S1–S3). Время указано в Almaty (UTC+5). Предполагается, что весь контент генерируется через промпты и отправляется вручную.
...
| | 12:00–12:30 | [S2](#s2) (Lead Scoring) | Отфильтровать 35 целевых профилей на неделю | Список из 35 prospects с оценкой ≥60 |
| | 18:00–18:30 | [P1](#p1) (Connection Note Generator) | Отправить 5 персонализированных connection requests (<250 chars, mobile-friendly) [→ repo] | 5 pending invites |
| **Вт** | 08:00–08:30 | [S1](#s1) (Boolean Search Generator) | Проверить tone-of-voice профиля на consistency | Профиль единообразен |
| | 12:00–12:30 | [P1](#p1) | Отправить 5 connection requests | 10 cumulative pending |
| | 18:00–18:30 | [P12](#p12) (SSI Analyzer) — *если конец недели* | Подвести итоги: acceptance rate, SSI delta, profile views | Базовые метрики зафиксированы |
| **Ср** | 08:00–09:00 | About Generator — ревизия | Доработать About на основе PAS + keywords | About оптимизирован |
| | 12:00–12:30 | [P1](#p1) | Отправить 5 connection requests | 15 cumulative pending |
| | 18:00–18:30 | Passive Consumption | Просмотр 20–30 лент, лайки (без комментариев) [→ repo] | Algorithmic trust растёт |
| **Чт** | 08:00–08:30 | [S2](#s2) (Lead Scoring) | Добавить 10–15 новых prospects в pipeline | Пополнена воронка |
| | 12:00–12:30 | [P1](#p1) | Отправить 5 connection requests | 20 cumulative pending |
| | 18:00–18:30 | [S3](#s3) (Content Calendar) | Проверить, не превышен ли soft cap pending invites (500–700) [→ repo] | Риск restriction минимален |
...
| | 12:00–12:30 | [P1](#p1) | Отправить 5 connection requests | 25 cumulative pending |
...
| **Вс** | 18:00–18:30 | [P12](#p12) (SSI Analyzer) | Сводка: новые connections, acceptance rate, SSI, profile views | Чёткая картина для корректировки |
...
| День | Время (Almaty) | Инструмент / Промпт | Действие | Ожидаемый результат |
|------|----------------|---------------------|----------|---------------------|
| **Пн** | 08:00–09:00 | [P4](#p4) (Comment Generator) + S2 (Lead Scoring) | Найти 3 поста целевой аудитории, сгенерировать 3 комментария (40+ слов, loop-back method) [→ repo] | 3 comments published |
| | 12:00–12:45 | [P1](#p1) (Connection Note Generator) | Отправить 10 персонализированных requests (70/неделю) | 10 pending invites |
| | 18:00–18:30 | [P2](#p2) (Follow-Up DM Generator) | Сгенерировать value-first follow-up для новых acceptances | 2–3 follow-up messages в очереди |
| **Вт** | 08:00–08:30 | [P4](#p4) | Оставить 3 комментария | 6 comments cumulative |
| | 12:00–12:45 | [P1](#p1) | Отправить 10 connection requests | 20 pending |
| | 18:00–18:30 | [P2](#p2) | Отправить follow-up (Day 3–4) для вчерашних acceptances | Тёплый touch, no pitch |
| **Ср** | 08:00–09:00 | [P10](#p10) (Full Post Generator) | Сгенерировать черновик поста (800–1000 слов, hook, framework, soft CTA) [→ repo] | Черновик готов к review |
| | 12:00–12:45 | [P1](#p1) | Отправить 10 connection requests | 30 pending |
| | 18:00–18:30 | [P4](#p4) | Оставить 3 комментария | 9 comments cumulative |
| **Чт** | 08:00–08:30 | [P9](#p9) (Content Mix Rotator) | Запланировать темы на следующую неделю (без конкретных заголовков — только тематические вёдра) | Content pipeline на 1 неделю вперёд |
| | 12:00–12:45 | [P1](#p1) | Отправить 10 connection requests | 40 pending |
| | 18:00–18:30 | [P2](#p2) | Отправить soft CTA (Day 7–10) для старых acceptances | Soft pitch или resource share |
| **Пт** | 08:00–08:30 | [P10](#p10) | Финальная редакция поста, подготовка к публикации (PDF/карусель если возможно — 1.9x performance [→ repo]) | Пост готов к публикации |
| | 12:00–12:45 | [P1](#p1) | Отправить 10 connection requests | 50 pending |
| | 18:00–18:30 | [P4](#p4) | Оставить 3 комментария | 12 comments cumulative |
| **Сб** | — | — | **Выходной. Нет активных действий.** | Предотвращение pattern detection [→ repo] |
| **Вс** | 10:00–10:30 | [P10](#p10) | **Публикация поста** (1/неделю) | Пост в ленте |
| | 18:00–19:00 | [P12](#p12) (SSI Analyzer) | Сводка: connections, acceptance rate, SSI, comments, impressions, saves | Data для decision framework |
...
| День | Время (Almaty) | Инструмент / Промпт | Действие | Ожидаемый результат |
|------|----------------|---------------------|----------|---------------------|
| **Пн** | 08:00–09:00 | [P4](#p4) (Comment Generator) | 5 комментариев (40+ слов, high-value, loop-back) [→ repo] | 5 comments published |
| | 12:00–12:30 | S2 (Lead Scoring) + [P1](#p1) (Connection Note Generator) | Отфильтровать 25 prospects, отправить 5 высококачественных requests | 5 targeted pending invites |
| | 18:00–19:00 | [P2](#p2) (Follow-Up DM Generator) | Сгенерировать 5-step sequence для hot leads (score ≥80) | Sequence готов |
| **Вт** | 08:00–09:00 | [P4](#p4) | 5 комментариев | 10 comments cumulative |
| | 12:00–12:30 | [P1](#p1) | 5 connection requests (качественные, с контекстом из комментариев) | 10 pending |
| | 18:00–18:30 | [P2](#p2) | Отправить Day 3–4 value message для вчерашних acceptances | Trust building |
| **Ср** | 08:00–09:00 | [P10](#p10) (Full Post Generator) | Сгенерировать пост или PDF carousel (1.9x performance [→ repo]) | Черновик готов |
| | 12:00–12:30 | [P4](#p4) | 5 комментариев | 15 comments cumulative |
| | 18:00–18:30 | [P2](#p2) | Day 7–10 soft CTA для старых acceptances | Soft pitch или resource share |
| **Чт** | 08:00–09:00 | P9 (Content Mix Rotator) + [P12](#p12) (SSI Analyzer) | Планирование контента + outline newsletter (если аудитория >500) [→ repo] | Pipeline на 2 недели |
| | 12:00–12:30 | [P1](#p1) | 5 connection requests | 15 pending |
| | 18:00–18:30 | [P2](#p2) | Day 14–30 follow-up или breakup message | Close или nurture list [→ repo] |
| **Пт** | 08:00–09:00 | [P4](#p4) | 5 комментариев | 20 comments cumulative |
| | 12:00–12:30 | [P1](#p1) | 5 connection requests | 20 pending |
| | 18:00–18:30 | [P6](#p6) (DM to Hiring Manager) | Генерация InMail / direct pitch для hot leads (если Premium) | InMail ready |
| **Сб** | — | — | **Выходной. Нет активных действий.** | Предотвращение pattern detection [→ repo] |
| **Вс** | 10:00–10:30 | [P10](#p10) | **Публикация поста** (1/неделю, возможно 2/неделю если capacity позволяет) | Пост + engagement |
| | 18:00–19:00 | [P12](#p12) (SSI Analyzer) | Полный анализ: connections, acceptance rate, reply rate, SSI, calls booked | Data для conversion optimization |
...
## 2.4 Символическая карта промптов (P1–[P12](#p12), S1–S3)

| ID | Название | Где используется | Источник |
|----|----------|------------------|----------|
| — | Headline Generator | Foundation, Growth | [→ inferred] |
| — | About Generator | Foundation, Growth | [→ repo] |
| [P1](#p1) | Connection Note Generator | Foundation, Growth, Conversion | [→ inferred] |
| [P2](#p2) | Follow-Up DM Generator | Growth, Conversion | [→ repo] |
| [P3](#p3) | Recruiter Message Generator | Growth, Conversion | [→ inferred] |
| [P4](#p4) | Comment Generator | Growth, Conversion | [→ inferred] |
| [P5](#p5) | Post Hook Generator | Growth, Conversion | [→ inferred] |
| [P6](#p6) | DM to Hiring Manager | Conversion | [→ inferred] |
| [P7](#p7) | Format Decision Engine | Growth, Conversion | [→ inferred] |
| [P8](#p8) | Tone & Angle Selector | Growth, Conversion | [→ inferred] |
| [P9](#p9) | Content Mix Rotator | Growth, Conversion | [→ inferred] |
| [P10](#p10) | Full Post Generator | Growth, Conversion | [→ repo] |
| [P11](#p11) | Performance Analyzer | Foundation, Growth, Conversion | [→ inferred] |
| [P12](#p12) | SSI Analyzer | Foundation, Growth, Conversion | [→ repo] |
| [S1](#s1) | Boolean Search Generator (Posts) | Все фазы | [→ inferred] |
| [S2](#s2) | Engagement Scoring / Lead Scoring | Все фазы | [→ repo] |
| [S3](#s3) | Content Calendar / Topic Discovery | Все фазы | [→ inferred] |
...
| **Acceptance rate 25–40%** | → Оставить текущий volume, усилить personalization ([P3](#p3) depth +1) | [→ inferred] |
| **Acceptance rate < 20%** | → Снизить connection requests на 50%, переключиться на commenting ([P5](#p5)) для warm-up [→ repo] | [→ repo] |
| **SSI вырос на +5+ за неделю** | → Увеличить posting frequency с 1 до 2/неделю (если capacity позволяет) | [→ inferred] |
| **SSI стагнирует 2+ недели** | → Аудит профиля (P1, P2) + усилить commenting ([P5](#p5)) до upper limit фазы | [→ inferred] |
...
| **3+ interview / discovery call на неделе** | → Connection requests = 0 на этой неделе, фокус на DM (P4, [P9](#p9)) и follow-up [→ inferred] | [→ inferred] |
| **0 replies на DMs за 2 недели** | → Аудит message tone ([S1](#s1)), увеличить value-first ratio, снизить CTA aggressiveness [→ repo] | [→ repo] |
| **Post saves > 10 за 48h** | → Удвоить commenting на авторах похожего контента (P5 + [P8](#p8)) | [→ inferred] |
| **Profile views < 100/неделю** | → Активировать profile view strategy (200–300 targeted views/день) + проверить headline ([P1](#p1)) [→ repo] | [→ repo] |
...
2. **Ручная отправка всего.** Даже если контент сгенерирован промптом ([P6](#p6), P3, P4), отправка — ручная. Автоматизация отправки = HIGH risk [→ repo].
...
5. **Персонализация > Volume.** Каждый connection request через P3. Каждый comment через [P5](#p5). Generic = 10–15% acceptance; personalized = 30–40%+ [→ repo].
6. **Content capacity ceiling.** Если `content_capacity` < 3 часов/неделю → посты 1/неделю, без newsletter. Если ≥ 5 часов → можно 2 поста + newsletter outline ([P12](#p12)) [→ repo].
...
|---------|---------|----------|
| Foundation → Growth (Day 15) | Профиль 100% + 150+ connections + acceptance rate ≥20% | P11 + [P12](#p12) |
| Growth → Conversion (Day 46) | 300+ connections + SSI ≥40 + acceptance rate ≥30% + ≥1 post опубликован | P11 + [P12](#p12) |
...
> [↑ К индексу алгоритмов](#index-algorithms)
...
**[S1](#s1)-A. LinkedIn Native Search (бесплатно)**
...
**[S1](#s1)-B. Google X-Ray (обходит алгоритмическую фильтрацию LinkedIn)**
...
**[S1](#s1)-C. Персонализированный Boolean Builder Prompt**
...
**[S2](#s2)-A. Коллеги и пиры (Peer Network)**
...
**[S2](#s2)-B. Рекрутеры и Talent Acquisition**
...
**[S2](#s2)-C. Hiring Managers (Engineering Managers, Directors)**
...
**[S2](#s2)-D. Boolean String Builder Prompt (люди)**
...
**[S3](#s3)-A. LinkedIn Trending Topics (нативный)**
...
**[S3](#s3)-B. LinkedIn Newsletters Deep Research**
...
**[S3](#s3)-C. Google Trends (бесплатно)**
...
**[S3](#s3)-D. Competitor Content Gap Analysis**
...
> **Принцип:** Инструменты — это не цель, а реализация алгоритмов. Каждый алгоритм (S1, S2, [S3](#s3)) может работать вручную; инструменты масштабируют.
...
| **Использование в GOS** | [S2](#s2) (поиск людей): Boolean search, lead lists, InMail. S1 (посты): поиск по хэштегам и keywords с фильтрами |
...
| **Использование в GOS** | [S3](#s3) (темы): алерты на "data engineering trends", "Apache Spark updates", имени конкурентов. S2 (люди): алерты на hiring announcements целевых компаний |
| **Risk** | SAFE |
...
| **Использование в GOS** | [S3](#s3) (темы): агрегация newsletters, блогов (Airbnb Engineering, Netflix Tech Blog, etc.), Reddit r/dataengineering. S1 (посты): RSS LinkedIn через n8n для автоматического сбора постов |
| **Risk** | SAFE |
...
| **Использование в GOS** | Автоматизация S1: сбор постов по RSS → AI-фильтрация (OpenAI node) → выдача в Telegram/Slack. Автоматизация [S3](#s3): сбор Google Trends → оповещение о росте. Автоматизация S2: интеграция с LinkedIn (через unofficial API — **ToS-violating, HIGH risk**) [→ repo] |
...
|----------|-------------------|-------------------------------|---------------------------|
| **[S1](#s1): Посты** | LinkedIn native search + Google X-Ray | Feedly RSS + n8n alerts | n8n + unofficial LinkedIn API (HIGH risk) [→ repo] |
| **[S2](#s2): Люди** | LinkedIn native search + Boolean strings | Sales Navigator trial + ручная квалификация | Sales Navigator + Apollo.io + n8n enrichment |
| **[S3](#s3): Темы** | Google Trends + Newsletter чтение | Google Alerts + Feedly + n8n aggregation | n8n + custom scraper + AI analysis |
...
|----------|-----------------------------------|-------|----------------|
| **[S1](#s1)** | `{persona}`, `{skills}`, `{target_audience}`, `{post_type}` | Очередь из 5–10 постов для комментирования | Execution: комментирование 5–10/day |
| **[S2](#s2)** | `{networking_goal}`, `{target_role}`, `{geography}`, `{seniority}` | Список 20–50 квалифицированных лидов | Execution: Warm DM protocol |
| **[S3](#s3)** | `{author_expertise}`, `{competitor_posts}`, `{industry_trends}` | 3–5 тем для постов на 2 недели | Execution: Content Creation |

**Feedback Loops:**
1. **S1 → [S2](#s2):** Если комментарий на посте автора получил ответ → автор переходит из S1 в [S2](#s2) как warm lead [→ inferred]
2. **S2 → [S3](#s3):** Если лид часто постит на определённую тему → тема добавляется в [S3](#s3) как potential gap или collaboration topic [→ inferred]
3. **[S3](#s3) → S1:** Если пост автора на тему из [S3](#s3) получил высокий engagement → тема добавляется в S1 как trending topic для комментирования [→ inferred]
...
|-------|----------|----------|------------|
| 08:00 | Проверить RSS/Feedly на новые посты | [S1](#s1) | Feedly / n8n |
| 08:15 | Проверить Google Alerts | [S3](#s3) | Google Alerts |
| 08:30 | LinkedIn native search: посты за последние 24 часа | [S1](#s1) | LinkedIn |
| 09:00 | Поиск 5–10 новых людей по Boolean string | [S2](#s2) | LinkedIn / Sales Navigator |
| 18:00 | Weekly review: какие посты/темы дали highest engagement? | S1, [S3](#s3) | Аналитика вручную |
...
> [↑ К индексу промптов](#index-prompts)
...
| Триггер | Порог | Действие |
|---------|-------|----------|
| Acceptance rate | <20% | -50% daily requests + пересмотреть Prompt [P1](#p1) (Target Profile Fit) [→ repo] |
...
- При component 4 (relationships) < 15: запрещено отправлять новые connection requests без audit [P1](#p1) [→ inferred]
...
### 9. Промпты P1–[P12](#p12): копирование и параметризация
...
- **Действие:** Скопируйте все операционные промпты (P1–[P12](#p12)) и генераторы профиля из соответствующих секций GOS в свой шаблон.
- **Параметризация:** замените `{input}` на ваши реальные данные (индустрия, география, роль).

### 10. Алгоритмы S1–[S3](#s3): тестирование
- **[S1](#s1) — Boolean Search Generator:** протестируйте 3 поисковых строки, проверьте количество результатов (цель: 500–2000 профилей).
- **[S2](#s2) — Engagement Scoring:** убедитесь, что скрипт/формула корректно считает веса (comments 2x likes, saves > likes) [→ repo].
- **[S3](#s3) — Content Calendar:** проверьте генерацию на 2 недели вперёд.
- **Decision framework:** если [S1](#s1) возвращает <100 результатов → расширьте Boolean strings (добавьте OR-клаузы); если >5000 → добавьте AND-фильтры (география, компания) [→ inferred].
...
|--------------|----------|---------------------|
| 09:00 | **[S1](#s1) — Boolean Search:** сгенерировать 1 таргетный сегмент (50–100 профилей) | [S1](#s1) |
| 09:30 | **[P4](#p4) — Comment Template:** написать 3 комментария к постам целевой аудитории (40+ слов каждый) | [P4](#p4) |
| 10:30 | **[P10](#p10) — Full Post Generator:** подготовить 1 пост (text-only, 800–1000 слов) | [P10](#p10) |
| 11:00 | **Публикация поста** (оптимальное время для EMEA: 10:00–12:00 ALMT) [→ inferred] |
| 12:00 | **[S2](#s2) — Engagement Scoring:** проскорить 20 профилей из S1 | [S2](#s2) |
| 12:30 | **[P1](#p1) — Connection Request:** отправить 10 персонализированных invites | [P1](#p1) |
| 13:00 | **[P2](#p2) — Follow-up DM:** отправить 3 follow-up сообщения (только тем, кто принял invite ранее) | [P2](#p2) |
| 14:00 | **Review:** зафиксировать метрики (просмотры поста, accepts, replies, profile views) | Google Sheets |
| 18:00 | **Второй раунд комментариев:** 3 комментария к постам лидеров мнений | [P4](#p4) |
| 20:00 | **Анализ первого поста:** dwell time, comments, likes (через LinkedIn Analytics) |
| 21:00 | **Планирование Day 2:** генерация через [S3](#s3) (Content Calendar) | [S3](#s3) |
...
- Если [S2](#s2) показывает низкий engagement score у таргетных профилей → расширить S1 (добавить смежные роли: Analytics Engineer, ML Engineer) [→ inferred].
...
### 16. Тюнинг системы
- **Prompt refinement:** если [P3](#p3) дал слабый пост → добавьте в input 2–3 примера постов, которые вам понравились (few-shot prompting) [→ inferred].
- **[S1](#s1) refinement:** если результаты поиска нерелевантны → проверьте Boolean operators (AND/OR/NOT) и добавьте исключения (`NOT recruiter`, `NOT HR`) [→ inferred].
...
- [ ] **9.** Промпты P1–[P12](#p12) и генераторы профиля скопированы в Google Sheets/Notion и параметризованы
- [ ] **10.** Алгоритмы S1–[S3](#s3) протестированы (S1: 500–2000 результатов; S2: корректные веса; [S3](#s3): 2 недели контента)
...
| Если… | То… | Источник |
|-------|-----|----------|
| [S1](#s1) не даёт результатов (<100 профилей) | Проверить Boolean strings → добавить OR-клаузы, убрать избыточные AND | [→ inferred] |
